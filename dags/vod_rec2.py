...
def mysql_hook(**context):
    ...
    vods = pd.read_sql('select * from vods_sumut', connection)
    conts = pd.read_sql('select * from contlog', connection)
...
def convert_to_json(**context):
    Precision = context["task_instance"].xcom_pull(task_ids="model_running_and_create", key="Precision")
    Recall = context["task_instance"].xcom_pull(task_ids="model_running_and_create", key="Recall")
    MAP = context["task_instance"].xcom_pull(task_ids="model_running_and_create", key="MAP")
    MAR = context["task_instance"].xcom_pull(task_ids="model_running_and_create", key="MAR")
    test_Diversity = context["task_instance"].xcom_pull(task_ids="model_running_and_create", key="test_Diversity")
    all_Diversity = context["task_instance"].xcom_pull(task_ids="model_running_and_create", key="all_Diversity")
    ...
    dict_data = {
        "columns":["Precision","Recall","MAP","MAR","test_Diversity","all_Diversity"],
        "values":[Precision, Recall, MAP, MAR, test_Diversity, all_Diversity]
    }
